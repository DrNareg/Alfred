...
def chat_page():
    ...
    messages = []
    try:
        local_tz = pytz.timezone(LOCAL_TIMEZONE) # Get your local timezone object
        for doc in reversed(list(messages_ref.stream())):
            data = doc.to_dict()
            ts_utc = data.get("timestamp") # This is likely a timezone-aware datetime object (UTC)

            if ts_utc:
                # Convert UTC timestamp to your local timezone
        ...
        print(f"Error processing messages: {e}") # Log the error for debugging
        # Handle the case where message processing fails, maybe return an empty list or show an error
        pass
