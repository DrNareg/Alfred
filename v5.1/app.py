...
@app.route("/jellyfin")
def jellyfin():
    # Replace this with your real Funnel URL
    funnel_url = "https://linktofunnel/"
    
    return redirect(funnel_url, code=301)
...
def daily_quote():
    ...
    try:
        # Fetch data from the ZenQuotes API with a timeout
        ...
        if data and isinstance(data, list) and len(data) > 0:
            quote_data = data[0]
            quote = quote_data.get('q', 'No quote text found.')
            author = quote_data.get('a', 'Unknown Author')
            app_logger.info("Successfully fetched daily quote.")
        else:
            app_logger.warning("ZenQuotes API returned empty or invalid data.")
            quote = "The API returned an unexpected response."
            author = "System"

    except requests.exceptions.RequestException as e:
        app_logger.error(f"Error fetching quote from ZenQuotes API: {e}")
        quote = "Could not connect to the quote service. Please check the network."
        author = "System"
    except Exception as e:
        app_logger.error(f"An unexpected error occurred in daily_quote route: {e}")
        quote = "An unexpected error occurred while fetching the quote."
        author = "System"
...
def transcribe_and_chat():
    ...
    try:
        ...
        # For audio chat, we might not want to pull text history, or maybe we do.
        # For simplicity, we'll start a fresh conversation context each time.
        # To include history, you'd fetch it here like in the /chat endpoint.
    ...
    try:
        ...
        voice = texttospeech.VoiceSelectionParams(
            language_code="en-US",
            name="en-US-Standard-J", # The voice you requested
